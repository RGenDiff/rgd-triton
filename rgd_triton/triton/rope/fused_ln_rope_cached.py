...
def _ln_transform_bwd(
    ...
    return drow_block_new


@triton.jit
def _fused_rope_ln_kernel_fwd(
        # -- inputs --
        q_ptr, 
        k_ptr, 
        sin_cos_ptr,
    ...
    # --- first apply the normalization ---
    new_row_block = _ln_transform_fwd(
                        norm_ptr,
                        row_block,
                        valid_mask,
                        seq_offsets,
    ...
    # -- now Apply the RoPE transformation to the block --
    new_row_block = _rope_transform_block(new_row_block, 
                                         sin_block, 
                                         cos_block,
                                         seq_offsets,
                                         chan_offsets,
...
def _fused_rope_ln_kernel_bwd(
    ...
    # -- first Apply the RoPE transformation to the block -- 
    drow_block = _rope_transform_block(drow_block, 
                                         sin_block, 
                                         cos_block,
                                         seq_offsets,
                                         chan_offsets,
    ...
    # -- then perform the ln backward pass --
    drow_block_new = _ln_transform_bwd(
                                        norm_ptr,
                                        row_block,
                                        drow_block,
                                        valid_mask,
...
def _fused_rope_ln_qk_fwd(q, k, pos_emb, seq_cutoff=None, heads_second=True, norm_eps=1e-6, kernel_cache=None):
    ...
    if (kernel_cache is not None) and (kernel_cache.fwd is not None):
        ...
        kfwd = _fused_rope_ln_kernel_fwd[grid](
            # -- inputs --
            q, 
            k, 
            pos_emb,
...
def _fused_rope_ln_qk_bwd(q, k, doq, dok, pos_emb, norm_buff, dq, dk, seq_cutoff=None, heads_second=True, norm_eps=1e-6, kernel_cache=None):

    assert q.stride(-1) == k.stride(-1) == doq.stride(-1) == dok.stride(-1) == pos_emb.stride(-1) == 1
    ...
    if (kernel_cache is not None) and (kernel_cache.bwd is not None):
        ...
        kbwd = _fused_rope_ln_kernel_bwd[grid](
            # -- inputs --
            q,
            k,
            pos_emb,
...
class FusedRoPEFunc(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, pos_emb, seq_cutoff=None, heads_second=True, norm_eps=1e-6, kernel_cache=None):
        """
        """
        # Make sure that the last dimension is contiguous
        q, k, pos_emb = [x if x.stride(-1) == 1 else x.contiguous() for x in [q, k, pos_emb]]
        qo, qk, norm_buff = _fused_rope_ln_qk_fwd(q, k, pos_emb, seq_cutoff=seq_cutoff, 
                                                  heads_second=heads_second,norm_eps=norm_eps,
                                                  kernel_cache=kernel_cache
                                                 )
        # Save the cache reference for the backward pass
        ctx.kernel_cache = kernel_cache
        # save the state for backward pass
        ctx.save_for_backward(q,k,pos_emb, norm_buff)
    ...
    def backward(ctx, doq, dok):
        q,k,pos_emb,norm_buff = ctx.saved_tensors
...
def triton_fused_ln_rope_fn(query,key,pos_emb, seq_cutoff=None, heads_second=True, norm_eps=1e-6, kernel_cache=None):
    return FusedRoPEFunc.apply(query,key,pos_emb, seq_cutoff, heads_second, norm_eps, kernel_cache)
