...
from rgd_triton.triton.rope.fused_ln_rope_cached import triton_fused_ln_rope_fn 
...
class Ref_Rotary(nn.Module):
    ...
    def forward(self, q, k, pos_emb, upcast=False):
        ...
        if self.fuse_ln:
            q = F.layer_norm(q, (q.shape[-1],), eps=1e-6)
            k = F.layer_norm(k, (k.shape[-1],), eps=1e-6)
...
def ref_rope_fn(query,key,pos_emb, seq_cutoff=None, heads_second=True, fused_ln=False, upcast=False):
    rr = Ref_Rotary(seq_cutoff=seq_cutoff, fuse_ln=fused_ln, heads_second=heads_second)
    rq, rk = rr.forward(query,key,pos_emb, upcast=upcast)
    return rq, rk

def make_inputs(B,H,NQ,NK,C,RDIM,EMB_LEN,ROT_FREQ=32, do_shift=True, heads_second=True, device="cuda", dtype=torch.float):
...
def get_tolerance(dtype):
    if dtype == torch.float32:
        ...
        return 1e-3, 1e-2 # accounting for multiplication
        #return 1e-2, 1e-3
    elif dtype == torch.bfloat16:
        return 1.6e-2, 1e-3 # from torch doc
        #return 1e-3, 1e-1 # accounting for multiplication
        #return 1e-2, 1e-3
    else:
...
def test_rope_head(seqlen, d, rdim, seq_cutoff, heads_second, dtype):
    ...
    qo, ko = triton_fused_ln_rope_fn(q, k, pos_emb.view(2,-1,pos_emb.shape[-1]), seq_cutoff=(seq_cutoff,seq_cutoff), heads_second=heads_second)
...
def test_rope_head_seqdiff(
    ...
    qo, ko = triton_fused_ln_rope_fn(q, k, pos_emb.view(2,-1,pos_emb.shape[-1]), seq_cutoff=(seq_cutoff_q,seq_cutoff_k), heads_second=heads_second)
...
def test_rope_head_race_condition(seqlen_q, seqlen_k, d, rdim, seq_cutoff, heads_second, dtype):
    ...
    qo0, ko0 = triton_fused_ln_rope_fn(q, k, pos_emb.view(2,-1,pos_emb.shape[-1]), seq_cutoff=(seq_cutoff_q,seq_cutoff_k), heads_second=heads_second)
